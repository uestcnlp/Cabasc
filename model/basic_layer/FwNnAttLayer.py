...
class FwNnAttLayer(object):
    ...
    def count_alpha(self, context, aspect, ctx_bitmap, alpha_adj=None):
        '''
        ...
        # alpha = softmax_mask(res_act, ctx_bitmap, 1)
        # alpha = sigmod_mask(res_act, ctx_bitmap, 1)
        alpha = normalizer(self.norm_type,res_act,ctx_bitmap,1)
        if alpha_adj is not None:
            alpha += alpha_adj
        return alpha

    def forward(self, context, aspect, ctx_bitmap, alpha_adj=None):
        '''
        ...
        alpha = self.count_alpha(
            context, aspect, ctx_bitmap, alpha_adj)
    ...
    def forward_wot_sum(self, context, aspect, ctx_bitmap, alpha_adj=None):
        '''
        Weight sum the context,
        line transform aspect,
        add two of them. 
        ...
        vec = tf.multiply(context, alpha_3dim)
        return vec, alpha
