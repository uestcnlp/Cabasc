...
class ConcatAttentionLayer:
    ...
    def count_alpha(self, context, aspect, ctx_bitmap):
        ...
        # adjust attention
        asp_3dim = tf.reshape(
            tf.tile(aspect, [1, mem_size]), [-1, mem_size, self.edim])
        ctx_cat_asp = tf.concat([context, asp_3dim],2)
        # gout = tf.matmul(context, asp_3dim)
        res_act = self.line_layer.forward(ctx_cat_asp)
        res_act = tf.reshape(res_act,[-1, mem_size])
