...
class batcher(object):
    ...
    def next_batch(self):
        ...
        for x in xrange(len(seq_lens)):
            ...
            r_l_pos = [x*-1 for x in lt_pos]
            rel_pos.extend(r_l_pos)
            rel_pos.extend(asp_pos)
            rel_pos.extend(rt_pos)
            rel_pos.extend(lest_pos)
        ...
        asp_bitmap = bitmap_by_padid(asps,self.pad_idx)
        # build the subs.
        asp_subs = []
        f_asp_subs = []
        b_asp_subs = []
        ...
        ret_data = {
            ...
            'rel_poses' : rel_poses,
            'pos_ids' : pos_ids,
            'max_asp_len' : asp_size,
            'max_sent_len' : max_len,
            'asp_bitmap': asp_bitmap
